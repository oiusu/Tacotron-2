...
class Synthesizer:
	...
	def synthesizev1(self, texts, basenames, out_dir, log_dir, mel_filenames):
		hparams = self._hparams
		cleaner_names = [x.strip() for x in hparams.cleaners.split(',')]
		...
		for i, mel in enumerate(mels):
			...
			if log_dir is not None:
				#save wav (mel -> wav)
				wav = audio.inv_mel_spectrogram(mel.T, hparams)
				wavPath = os.path.join(log_dir, 'wavs/wav-{}-mel.wav'.format(basenames[i]))
				wavPath = os.path.join(os.getcwd(),wavPath)
				wavPaths.append(wavPath)
				audio.save_wav(wav,wavPath , sr=hparams.sample_rate)
